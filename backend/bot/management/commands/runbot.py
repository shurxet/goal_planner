...
from bot.tg import TgClient, Message, MemoryStorage
...
class Command(BaseCommand):
    ...
    def handle_goals_list(self, msg: Message, tg_user: TgUser):
        resp_goals: list[str] = [f'#{goal.id} {goal.title}'for goal in Goal.objects.filter(user_id=tg_user.user_id)]
        if resp_goals:
    ...
    def handle_goal_categories_list(self, msg: Message, tg_user: TgUser):
        resp_categories: list[str] = [
            f'#{cat.id} {cat.title}' for cat in GoalCategory.objects.filter(
